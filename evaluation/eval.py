...
def bleu(ref_token, cand_token):
    ...
    for i in range(1, 5):
        ...
        for r, c in zip(ref_token, cand_token):
            result["bleu-%d" % i].append(
                nltk.translate.bleu_score.sentence_bleu([r], c, weights=tuple([1. / i for j in range(i)])))
...
if __name__ == '__main__':
    ...
    task = 'mo2st'
    lang = 'zh'
    truth_file = 'test_content.txt'
    result_file1 = '/data2/data/wxx/LLaMA-Factory/data/ccstories/result_qwen3_0.6b-hebing.txt'
    result_file1_ft = '/data2/data/wxx/LLaMA-Factory/data/ccstories/result_qwen3_0.6b-ft-4000.txt'
    result_file2 = '/data2/data/wxx/LLaMA-Factory/data/ccstories/filter/result_qwen3_1.7b.txt'
    result_file2_ft = '/data2/data/wxx/LLaMA-Factory/data/ccstories/result_qwen3_1.7b-ft.txt'
    result_file3 = '/data2/data/wxx/LLaMA-Factory/data/ccstories/filter/result_qwen3_4b.txt'
    result_file3_ft = '/data2/data/wxx/LLaMA-Factory/data/ccstories/result_qwen3_4b-ft.txt'
    result_file4 = '/data2/data/wxx/LLaMA-Factory/data/ccstories/filter/result_qwen3_8b.txt'
    result_file4_ft = '/data2/data/wxx/LLaMA-Factory/data/ccstories/result_qwen3_8b-ft.txt'
    f=open('test_input.txt','r',encoding="utf-8")
    with open(truth_file, encoding="utf-8") as fin:
        if task == "mo2st":
            # ipt = [d["标题"].strip() for d in data]
            # truth = [d["story"].strip() for d in data]
            ipt=[]
            truth=[]
            for line in f.readlines():
                ipt.append(line.replace("标题：", "").replace("开头：","").replace("续写：",""))
                # post = line.find('title:')
                # if post != -1:
                #     ipt.append(line[6:].strip())          
            for line in fin.readlines():
                ...
                #     continue
                # else:
                #     truth.append(line[15:315].strip())
    ...
    result_list = [result_file1,result_file1_ft,result_file2,result_file2_ft,result_file3,result_file3_ft,result_file4,result_file4_ft]
    ...
    for name in result_list:
        with open(name, "r", encoding="utf-8") as fin:
            ...
            cand = []
            for line in fin.readlines():
                cand.append(line[:500])
                # post = line.find('result:')
                # if post != -1:
                #     cand.append(line[22:].strip())
                #     continue
                # post = line.find('标题：')
                # if post == -1:
