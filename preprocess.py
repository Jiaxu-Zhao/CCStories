...
def preprocess():
    ...
    # parser.add_argument('--data_path', default='data/train_raw/train_raw', type=str, required=False, help='数据集存放位置')
    parser.add_argument('--data_path', default='LOT/train.jsonl', type=str, required=False, help='数据集存放位置')
    parser.add_argument('--save_path', default='LOT/train.pkl', type=str, required=False, help='对训练数据集进行tokenize之后的数据存放位置')
    ...
    #         try:
    #             title = lines[1][8:-3].strip()    # 取出标题
    #             lines = lines[2:]   # 取出正文内容
    #         except IndexError:
    ...
    fp = jsonlines.open(args.data_path, 'r')
    for article in fp:
        title = ""
        article = article["story"].replace("<mask>", "")
        print(article)
        title_ids = tokenizer.encode(title, add_special_tokens=False)
        article_ids = tokenizer.encode(article, add_special_tokens=False)
        token_ids = title_ids + [sep_id] + article_ids + [eod_id]
    ...
    # articles = json.load(open(args.data_path, 'r'))
