...
def set_args():
    ...
    parser.add_argument('--vocab_path', default='vocab/chinese_vocab.model', type=str, required=False,
                        help='sp模型路径')
    parser.add_argument('--model_config', default='./CPM-large/config.json', type=str, required=False,
                        help='需要从头训练一个模型时，模型参数的配置文件')
    parser.add_argument('--train_path', default='LOT/train.pkl', type=str, required=False, help='经过预处理之后的数据存放路径')
    parser.add_argument('--max_len', default=200, type=int, required=False, help='训练时，输入数据的最大长度')
    parser.add_argument('--log_path', default='log/raw.log', type=str, required=False, help='训练日志存放位置')
    ...
    parser.add_argument('--save_model_path', default='model/LOT', type=str, required=False,
                        help='模型输出路径')
    parser.add_argument('--pretrained_model', default='', type=str, required=False,
                        help='预训练的模型的路径')
    #train_losses:[5.2737],ppl:[195.13663393]
    #train_losses:[4.5325],ppl:[92.9907476]
    #train_losses: [3.5618], ppl: [35.2265479]
...
def train_epoch(model, train_dataloader, optimizer, scheduler, logger,
    ...
    for batch_idx, (input_ids, labels) in enumerate(train_dataloader):
        ...
        try:
            ...
            if (batch_idx + 1) % args.log_step == 0:
                logger.info(
                    "batch {} of epoch {}, ppl{}, loss {}, batch_acc {}, lr {}".format(
                        batch_idx + 1, epoch + 1, np.exp(loss.item() * args.gradient_accumulation_steps),loss.item() * args.gradient_accumulation_steps, batch_acc, scheduler.get_lr()))
    ...
    epoch_mean_ppl = total_ppl/ len(train_dataloader)
    epoch_mean_acc = epoch_correct_num / epoch_total_num
    logger.info(
        "epoch {}: ppl{},loss {}, predict_acc {}".format(epoch + 1,epoch_mean_ppl, epoch_mean_loss, epoch_mean_acc))
...
def train(model, logger, train_dataset, args):
    ...
    for epoch in range(args.epochs):
        ...
        logger.info("train loss list:{},ppl:{}\n".format(train_losses,np.exp(train_losses)))

    logger.info('training finished')
    logger.info("train_losses:{},ppl:{}".format(train_losses,np.exp(train_losses)))
    logger.info("ppl:{}".format(train_losses))
